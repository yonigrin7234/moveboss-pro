...
export default function PostPickupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // IMPORTANT: Only use columns that definitely exist in production database
      // Use pickup_*, delivery_* columns (NOT origin_*, destination_*)
      // Use cubic_feet_estimate (NOT estimated_cuft)
      // Use posting_status (NOT load_status)
      const { data: insertedLoad, error: insertError } = await supabase.from('loads').insert({
        ...
        // Origin - use pickup_* columns (these definitely exist)
        ...
        // Destination - use delivery_* columns (these definitely exist)
        ...
        // Load details - use cubic_feet_estimate (definitely exists)
        cubic_feet_estimate: parseFloat(formData.cubic_feet) || 0,
