 ...
 * (excludes start date, includes end date)
 */
export function getCalendarDaysBetween(startDate: Date, endDate: Date): number {
  const start = normalizeDate(startDate);
  const end = normalizeDate(endDate);
...
export function getDaysUntil(targetDate: Date | string): number {
  ...
  return getCalendarDaysBetween(today, target);
}

/**
 * Get business days until a target date from today
 */
export function getBusinessDaysUntil(targetDate: Date | string): number {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
