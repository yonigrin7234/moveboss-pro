...
interface TripMessageBadgeProps {
  tripId: string;
  /** Size variant */
  size?: 'sm' | 'default';
...
/**
 * Client component to show a message icon with unread badge for a trip.
 * Can be embedded in server components.
 */
export function TripMessageBadge({
  tripId,
  size = 'default',
  showWhenEmpty = true,
}: TripMessageBadgeProps) {
  const { unreadCount } = useSingleEntityUnreadCount('trip', tripId);
  ...
  return (
    ...
    >
      <Link href={`/dashboard/trips/${tripId}?tab=messages`}>
 ...
 */
export function TripMessageIndicator({
  tripId,
  className = '',
}: {
  tripId: string;
  ...
  return (
    <Link
      href={`/dashboard/trips/${tripId}?tab=messages`}
