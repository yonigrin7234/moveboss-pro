...
export async function createLoad(input: NewLoadInput, userId: string): Promise<Load> {
  ...
  // For "My Customer" (own_customer) loads, auto-assign the user's workspace company
  // This ensures all own-customer loads belong to the user's company, not left unassigned
  let finalCompanyId = input.company_id;
  if (input.load_source === 'own_customer' && !input.company_id) {
    ...
    if (workspaceCompany) {
      finalCompanyId = workspaceCompany.id;
    }
  }

  // Verify company ownership only if a partner company is specified
  // (workspace company doesn't need verification - it's the user's own company)
  if (input.company_id && input.load_source === 'partner') {
